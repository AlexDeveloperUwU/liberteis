    ...
    <div class="relative">
      <form @submit.prevent="handleSubmit" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="order-2 lg:order-1">
          <div class="bg-background-100 p-6 rounded-lg border-[1.5px] border-background-300 shadow-[0_4px_20px_-2px_rgba(0,0,0,0.1)] h-full">
            ...
            <div class="bg-background-50 p-5 rounded-lg border border-background-200">
              <div class="space-y-4">
                ...
                <div v-if="formData.info" class="flex items-start p-3 bg-background-100 rounded-lg border border-background-200">
        ...
        <div class="order-1 lg:order-2">
          <div class="bg-background-100 p-6 rounded-lg border-[1.5px] border-background-300 shadow-[0_4px_20px_-2px_rgba(0,0,0,0.1)] h-full">
              ...
              <div class="bg-background-50 p-4 rounded-lg border border-background-200">
                ...
                <div class="mb-4">
                  ...
                  <div class="relative group">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                      <Calendar class="w-5 h-5 text-primary-600 group-hover:text-primary-600 transition-colors duration-200" />
                    </div>
                    <input v-model="formData.title" id="title" type="text" class="block w-full pl-10 pr-3 py-2 border border-background-300 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent hover:border-primary-300 transition-all duration-200 text-text-950 font-medium" :class="{
                      'border-accent-500 ring-1 ring-accent-300': errors.title && !formSubmitted,
                    }" :placeholder="t('pages.dash.eventsForm.form.placeholders.title') || 'Nombre del evento'" required />
                    <div class="absolute inset-y-0 right-3 flex items-center">
                      <CheckCircle2 v-if="(!errors.title || formSubmitted) && formData.title && formData.title.length >= 3" class="w-5 h-5 text-primary-500 animate-fadeIn" />
                      <XCircle v-else-if="(formData.title || touchedFields.title) && !formSubmitted" class="w-5 h-5 text-accent-500 animate-fadeIn" />
                    </div>
                  </div>
                </div>
                ...
                <div class="mb-4">
                  ...
                  <div class="relative">
                    <Listbox v-model="selectedCategory">
                      <div class="relative">
                        <ListboxButton class="relative w-full pl-10 pr-10 py-2 border border-background-300 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent hover:border-primary-300 transition-all duration-200 bg-background-50 text-left" :class="{
                          'border-accent-500 ring-1 ring-accent-300': errors.category && !formSubmitted,
                        }">
                          <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <Bookmark class="w-5 h-5 text-primary-600" />
                          </div>
                          <span class="block truncate" :class="!selectedCategory ? 'text-text-400' : 'text-text-950 font-medium'">
                            {{
                          ...
                          <span class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                            <div class="flex items-center">
                              <XCircle v-if="errors.category && !formSubmitted" class="w-5 h-5 text-accent-500 mr-2 animate-fadeIn" />
                              <ChevronDown class="w-5 h-5 text-text-400" />
                            </div>
                          </span>
                        </ListboxButton>
                        <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                          <ListboxOptions class="absolute z-10 mt-1 w-full bg-background-50 border border-background-300 rounded-md shadow-lg max-h-60 overflow-auto focus:outline-none sm:text-sm origin-top-right">
                            <ListboxOption v-for="category in categories" :key="category._id || category.id" :value="category._id || category.id" v-slot="{ active, selected }">
                              <li :class="[
                                selected
                ...
                <div class="mb-4">
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    <div class="relative group">
                      <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <Clock class="w-5 h-5 text-primary-600 group-hover:text-primary-600 transition-colors duration-200" />
                      </div>
                      <input v-model.number="formData.hours" id="hours" type="number" min="0" max="24" step="1" class="block w-full pl-10 pr-3 py-2 border border-background-300 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent hover:border-primary-300 transition-all duration-200 text-text-950 font-medium no-spinner" :placeholder="t('pages.dash.eventsForm.form.placeholders.hours')" @input="updateDuration" />
                      <div class="absolute inset-y-0 right-3 flex items-center">
                    ...
                    <div class="relative group">
                      ...
                      <input v-model.number="formData.minutes" id="minutes" type="number" min="0" max="59" step="5" class="block w-full pl-10 pr-3 py-2 border border-background-300 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent hover:border-primary-300 transition-all duration-200 text-text-950 font-medium no-spinner" :class="{ 'border-accent-500 ring-1 ring-accent-300': errors.duration && !formSubmitted }" :placeholder="t('pages.dash.eventsForm.form.placeholders.minutes')" @input="updateDuration" />
                      <div class="absolute inset-y-0 right-3 flex items-center">
                ...
                <div class="mb-4">
                  ...
                  <div class="relative group">
                    <textarea v-model="formData.info" id="info" rows="5" maxlength="500" class="block w-full p-3 border border-background-300 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent hover:border-primary-300 transition-all duration-200 text-text-950" :class="{ 'border-accent-500 ring-1 ring-accent-300': errors.info && !formSubmitted }" :placeholder="t('pages.dash.eventsForm.form.placeholders.info') || 'Información del evento'
                      "></textarea>
                    <div class="absolute bottom-3 right-3 flex items-center">
                      <span class="text-xs text-text-500 mr-2">{{ formData.info.length }}/500</span>
                      <CheckCircle2 v-if="(!errors.info || formSubmitted) && formData.info && formData.info.length >= 3" class="w-5 h-5 text-primary-500 animate-fadeIn" />
                      <XCircle v-else-if="(formData.info || touchedFields.info) && !formSubmitted && errors.info" class="w-5 h-5 text-accent-500 animate-fadeIn" />
                    </div>
                  </div>
                </div>
                ...
                <div class="mb-4">
                  ...
                  <div class="relative group">
                    <input ref="fileInput" type="file" id="cover" accept="image/*" class="hidden" @change="handleFileUpload" />

                    <div class="w-full" @dragover.prevent="isDragging = true" @dragleave.prevent="isDragging = false" @drop.prevent="onDrop">
                      <div :class="[
            ...
            <div class="flex justify-end items-center mt-6 gap-2">
              <button type="button" class="h-10 px-4 rounded-lg text-sm font-medium shadow-sm flex items-center justify-center bg-background-100 text-text-700 border border-background-300 hover:bg-background-200 transition-colors duration-150 cursor-pointer" @click="$router.push({ name: 'dashEvents' })">
                <X class="w-4 h-4 mr-2" />
                {{ t("pages.dash.eventsForm.common.cancel") }}
              </button>
              <button type="submit" class="h-10 px-5 rounded-lg text-sm font-medium shadow-sm flex items-center justify-center transition-colors duration-150 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed" :class="{
    ...
    <Teleport to="body">
      <Transition enter-active-class="transition-opacity duration-300 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition-opacity duration-300 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <div v-if="showImageModal && (imagePreview || formData.coverUrl)" class="fixed inset-0 z-50">
          <div class="fixed inset-0 bg-background-950/85 dark:bg-background-50/85 transition-opacity duration-300"></div>

          <div class="fixed inset-0 overflow-y-auto">
            <div class="flex min-h-full items-center justify-center p-4 text-center">
              <div class="relative w-full max-w-3xl mx-auto">
                <div class="relative bg-background-50 dark:bg-background-100 rounded-lg shadow-xl p-2">
                  <div class="flex justify-between items-center p-4 border-b border-background-200 dark:border-background-300">
                    <h3 class="text-lg font-semibold text-text-900 dark:text-text-800">
                      {{ t("pages.dash.eventsForm.form.labels.cover") }}
                    </h3>
                    <button @click="showImageModal = false" class="rounded-md p-2 text-text-700 dark:text-text-700 hover:bg-background-100 dark:hover:bg-background-200 hover:text-text-900 dark:hover:text-text-900 transition-colors">
                      <X class="w-5 h-5" />
                    </button>
                  </div>

                  <div class="p-4">
                    <img :src="imagePreview || formData.coverUrl" alt="Event cover fullscreen" class="max-h-[70vh] mx-auto object-contain rounded-lg" />
                  </div>

                  <div class="bg-background-100 dark:bg-background-200 px-6 py-4 flex justify-end gap-2 rounded-b-lg">
                    <button @click="showImageModal = false" class="inline-flex justify-center rounded-md bg-background-50 dark:bg-background-100 px-3 py-2 text-sm font-semibold text-text-800 dark:text-text-700 shadow-sm ring-1 ring-inset ring-background-300 dark:ring-background-400 hover:bg-background-100 dark:hover:bg-background-200 transition-colors">
...
const updateSelectedCategoryInfo = () => {
  if (selectedCategory.value && categories.value.length > 0) {
    selectedCategoryInfo.value = categories.value.find(cat => (cat._id === selectedCategory.value) || (cat.id === selectedCategory.value)) || null;
  } else {
...
const selectedCategoryName = computed(() => {
  ...
  const category = categories.value.find((cat) => (cat._id === selectedCategory.value) || (cat.id === selectedCategory.value));
