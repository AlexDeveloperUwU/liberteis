            ...
            <div class="bg-background-50 p-5 rounded-lg border border-background-200">
              <div class="space-y-4">
                ...
                <template v-if="selectedEventInfo">
                  ...
                  <div class="flex items-center p-3 bg-background-100 rounded-lg border border-background-200" v-if="selectedEventInfo.duration">
        ...
        <div class="order-1 lg:order-2">
          <div class="bg-background-100 p-6 rounded-lg border-[1.5px] border-background-300 shadow-[0_4px_20px_-2px_rgba(0,0,0,0.1)] h-full">
              ...
              <div class="bg-background-50 p-4 rounded-lg border border-background-200">
                    ...
                    <Listbox v-model="selectedEvent" :disabled="isEditMode">
                      <div class="relative">
                        <ListboxButton class="relative w-full pl-10 pr-10 py-2 border border-background-300 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent hover:border-primary-300 transition-all duration-200 bg-background-50 text-left" :class="{
                          ...
                          <span class="block truncate" :class="!selectedEvent ? 'text-text-400' : 'text-text-950 font-medium'">
                            {{ selectedEventName || t("pages.dash.bookingForm.form.placeholders.selectEvent") }}
                          </span>
                          <span class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                            <div class="flex items-center">
                              <XCircle v-if="errors.event && !formSubmitted" class="w-5 h-5 text-accent-500 mr-2 animate-fadeIn" />
                        ...
                        <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                          <ListboxOptions class="absolute z-10 mt-1 w-full bg-background-50 border border-background-300 rounded-md shadow-lg max-h-60 overflow-auto focus:outline-none sm:text-sm origin-top-right">
                            <ListboxOption v-for="event in events" :key="event._id || event.id" :value="event._id || event.id" v-slot="{ active, selected }">
                              <li :class="[
                                selected
                    ...
                    <Listbox v-model="selectedSpace" :disabled="!selectedEvent && !isEditMode">
                      <div class="relative">
                        <ListboxButton class="relative w-full pl-10 pr-10 py-2 border border-background-300 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent hover:border-primary-300 transition-all duration-200 bg-background-50 text-left" :class="{
                          ...
                          <span class="block truncate" :class="!selectedSpace ? 'text-text-400' : 'text-text-950 font-medium'">
                            {{
                          ...
                          <span class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                            <div class="flex items-center">
                              <XCircle v-if="errors.space && !formSubmitted" class="w-5 h-5 text-accent-500 mr-2 animate-fadeIn" />
                              <ChevronDown class="w-5 h-5 text-text-400" />
                            </div>
                          </span>
                        </ListboxButton>
                        <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                          <ListboxOptions class="absolute z-10 mt-1 w-full bg-background-50 border border-background-300 rounded-md shadow-lg max-h-60 overflow-auto focus:outline-none sm:text-sm origin-top-right">
                            <ListboxOption v-for="space in availableSpaces" :key="space._id || space.id" :value="space._id || space.id" v-slot="{ active, selected }">
                              <li :class="[
                                selected
                                  ? 'bg-primary-100 border-l-primary-500 text-primary-800'
                                  : active
                ...
                <div class="mb-4">
                  ...
                  <div class="relative group">
                    ...
                    <input v-model="formData.bookingDate" id="bookingDate" type="datetime-local" class="block w-full pl-10 pr-3 py-2 border border-background-300 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent hover:border-primary-300 transition-all duration-200 text-text-950 font-medium" :class="{
                      'border-accent-500 ring-1 ring-accent-300': errors.bookingDate && !formSubmitted,
                    }" required />
                    <div class="absolute inset-y-0 right-3 flex items-center">
                      <CheckCircle2 v-if="(!errors.bookingDate || formSubmitted) && formData.bookingDate" class="w-5 h-5 text-primary-500 animate-fadeIn" />
                      <XCircle v-else-if="(formData.bookingDate || touchedFields.bookingDate) && !formSubmitted" class="w-5 h-5 text-accent-500 animate-fadeIn" />
                    </div>
                  </div>
                </div>
                ...
                <div class="mb-4">
                  ...
                  <div class="relative group">
                    <textarea v-model="formData.info" id="info" rows="4" maxlength="500" class="block w-full p-3 border border-background-300 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent hover:border-primary-300 transition-all duration-200 text-text-950" :class="{ 'border-accent-500 ring-1 ring-accent-300': errors.info && !formSubmitted }" :placeholder="t('pages.dash.bookingForm.form.placeholders.info')"></textarea>
                    <div class="absolute bottom-3 right-3 flex items-center">
                      ...
                      <CheckCircle2 v-if="(!errors.info || formSubmitted)" class="w-5 h-5 text-primary-500 animate-fadeIn" />
                      <XCircle v-else-if="errors.info && !formSubmitted" class="w-5 h-5 text-accent-500 animate-fadeIn" />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="flex justify-end items-center mt-6 gap-2">
              <button type="button" class="h-10 px-4 rounded-lg text-sm font-medium shadow-sm flex items-center justify-center bg-background-100 text-text-700 border border-background-300 hover:bg-background-200 transition-colors duration-150 cursor-pointer" @click="$router.push({ name: returnRoute })">
            ...
            <div class="flex min-h-full items-center justify-center p-4 text-center">
              <div class="relative w-full max-w-3xl mx-auto">
                <div class="relative bg-background-50 dark:bg-background-100 rounded-lg shadow-xl p-2">
                  ...
                  <div class="p-4">
                    <img :src="selectedEventInfo.coverUrl" alt="Event cover fullscreen" class="max-h-[70vh] mx-auto object-contain rounded-lg" />
                  </div>
