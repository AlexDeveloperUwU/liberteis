  ...
  <nav class="bg-gray-800 text-white flex items-center justify-between px-6 py-3 shadow-md">
    ...
    <div class="flex items-center gap-4">
      <div class="relative" @click.stop>
        <button @click.stop="toggleLanguageMenu" class="flex items-center gap-1 text-gray-400 hover:text-white py-1 px-2 rounded hover:bg-gray-700 transition">
        ...
        <div v-if="showLanguageMenu" class="absolute right-0 mt-1 w-32 bg-gray-700 rounded shadow-lg py-1 z-10">
          <button @click="setLocale('en')" class="block w-full text-left px-4 py-2 text-sm hover:bg-gray-600 transition" :class="{ 'bg-gray-600': locale === 'en' }">
            English
          </button>
          <button @click="setLocale('es')" class="block w-full text-left px-4 py-2 text-sm hover:bg-gray-600 transition" :class="{ 'bg-gray-600': locale === 'es' }">
            Español
          </button>
          <button @click="setLocale('gl')" class="block w-full text-left px-4 py-2 text-sm hover:bg-gray-600 transition" :class="{ 'bg-gray-600': locale === 'gl' }">
            Galego
          </button>
        </div>
      </div>
