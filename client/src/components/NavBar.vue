  ...
  <nav class="navbar fixed top-0 left-0 w-full z-50 bg-background-100 border-b-[1.5px] border-background-300">
    <div class="flex items-center justify-between px-6 py-3">
      <div class="flex items-center gap-3">
        <button v-if="currentLayout === 'dashboard'" @click="$emit('toggle-sidebar')" class="flex items-center justify-center w-8 h-8 rounded-md bg-background-200 hover:bg-background-300 text-text-800 hover:shadow-md transition-all duration-200">
        ...
        <Menu as="div" class="relative inline-block text-left">
          <div>
            <MenuButton class="w-8 h-8 rounded-full overflow-hidden border-2 border-primary-400 hover:border-primary-500 transition-colors flex items-center justify-center bg-white p-0">
              <img :src="getImageUrl('default', 'png')" alt="User Avatar" class="block w-full h-full object-cover object-center m-0 p-0" />
            </MenuButton>
          </div>
          <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
            <MenuItems class="absolute right-0 mt-3 w-48 origin-top-right rounded-md bg-background-100 border border-background-300 shadow-lg">
              <div class="py-1">
                ...
                <button @click="handleLogin" class="w-full text-left px-4 py-2 text-sm transition-all duration-150 border-l-[3px] border-transparent flex items-center" :class="[
                  active
                    ? 'border-l-primary-500 text-primary-600 bg-primary-50'
                    : 'text-text-800 hover:bg-primary-50 hover:text-primary-600 hover:border-l-primary-300',
                ]">
