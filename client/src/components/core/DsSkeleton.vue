...
const style = (isLast) => ({
  ...
  height:
    props.height ??
    (props.variant === "text" ? "0.875rem" : props.variant === "circle" ? (props.width ?? "2.5rem") : "1rem"),
});
</script>

<template>
  <div v-if="variant === 'text'" class="flex flex-col gap-2">
    <div v-for="i in lines" :key="i" class="rounded-md bg-background-300 animate-pulse" :style="style(i === lines)" />
  </div>
  <div
    v-else
    class="bg-background-300 animate-pulse"
