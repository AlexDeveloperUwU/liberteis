<template>
  <Teleport to="body">
    <TransitionGroup enter-active-class="transition-opacity duration-300 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition-opacity duration-300 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
        ...
        <div class="fixed inset-0 z-50">
          ...
          <div class="fixed inset-0 overflow-y-auto">
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
              <div class="relative w-full transform rounded-lg bg-background-50 dark:bg-background-100 text-left shadow-xl transition-all duration-300 sm:my-8 opacity-100 scale-100" :class="getModalWidth(modal.width)">
                <div class="border-b border-background-200 dark:border-background-300">
                  <div class="flex items-center justify-between p-4">
                    ...
                    <button @click="modalStore.removeModal(modal.id)" class="rounded-md p-2 text-text-700 dark:text-text-700 hover:bg-background-100 dark:hover:bg-background-200 hover:text-text-900 dark:hover:text-text-900 transition-colors">
                  ...
                  <div class="text-sm text-text-800 dark:text-text-700 space-y-4">
                    <div v-if="modal.data?.coverUrl" class="mb-4">
                      <div class="flex flex-col items-center">
                        <div class="relative overflow-hidden rounded-lg border border-background-300 bg-background-200 dark:bg-background-300" style="max-width: 15%; height: auto;">
                          <img :src="modal.data.coverUrl" alt="Imagen del evento" class="w-full object-contain aspect-[9/16]" />
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="flex items-center p-3 bg-background-100 rounded-lg border border-background-200">
                      ...
                      <div>
                        <p class="text-xs text-text-600">{{ $t('components.modals.bookingDetails.event') }}</p>
                        <p class="text-text-900 font-medium">{{ modal.data?.eventName || $t('components.modals.common.notAvailable') }}</p>
                      </div>
                    </div>

                    <div v-if="modal.data?.eventDescription" class="flex items-start p-3 bg-background-100 rounded-lg border border-background-200">
                    ...
                    <div v-if="!modal.data?.coverUrl && modal.data?.eventDescription" class="flex items-start p-3 bg-background-100 rounded-lg border border-background-200">
                      <div class="bg-primary-100 p-2 rounded-full border-primary-300 mr-3 mt-1 flex-shrink-0">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-primary-700">
                          <path stroke-linecap="round" stroke-linejoin="round" d="m11.25 11.25.041-.02a.75.75 0 0 1 1.063.852l-.708 2.836a.75.75 0 0 0 1.063.853l.041-.021M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9-3.75h.008v.008H12V8.25Z" />
                        </svg>
                    ...
                    <div v-if="modal.data?.bookingInfo" class="flex items-start p-3 bg-background-100 rounded-lg border border-background-200">
                      <div class="bg-primary-100 p-2 rounded-full border-primary-300 mr-3 mt-1 flex-shrink-0">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-primary-700">
                    ...
                    <div class="flex items-center p-3 bg-background-100 rounded-lg border border-background-200">
                      ...
                      <div>
                        <p class="text-xs text-text-600">{{ $t('components.modals.bookingDetails.dateTime') }}</p>
                        <p class="text-text-900 font-medium">{{ modal.data?.dateTime || $t('components.modals.common.notAvailable') }}</p>
                      </div>
                    </div>
                    ...
                    <div class="flex items-center p-3 bg-background-100 rounded-lg border border-background-200">
                      ...
                      <div>
                        <p class="text-xs text-text-600">{{ $t('components.modals.bookingDetails.duration') }}</p>
                        <p class="text-text-900 font-medium">{{ modal.data?.duration || $t('components.modals.common.notAvailable') }}</p>
                      </div>
                    </div>
                    ...
                    <div class="flex items-center p-3 bg-background-100 rounded-lg border border-background-200">
                      ...
                      <div>
                        <p class="text-xs text-text-600">{{ $t('components.modals.bookingDetails.space') }}</p>
                        <p class="text-text-900 font-medium">{{ modal.data?.space || $t('components.modals.common.notAvailable') }}</p>
                      </div>
                    </div>
                    ...
                    <div class="flex items-center p-3 bg-background-100 rounded-lg border border-background-200">
                      ...
                      <div>
                        <p class="text-xs text-text-600">{{ $t('components.modals.bookingDetails.category') }}</p>
                        <p class="text-text-900 font-medium">{{ modal.data?.category || $t('components.modals.common.notAvailable') }}</p>
                      </div>
                    </div>
                    ...
                    <div class="flex items-center p-3 bg-background-100 rounded-lg border border-background-200">
                      ...
                      <div>
                        <p class="text-xs text-text-600">{{ $t('components.modals.bookingDetails.bookedBy') }}</p>
                        <p class="text-text-900 font-medium">{{ modal.data?.bookedBy || $t('components.modals.common.notAvailable') }}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="bg-background-100 dark:bg-background-200 px-6 py-4 sm:flex sm:flex-row sm:gap-2">
                  <!-- Asegurarse de que los elementos de la derecha estén bien posicionados -->
                  <div class="w-full sm:w-auto sm:ml-auto flex gap-2 justify-end">
                    <!-- Acciones o botón de cerrar -->
                    <template v-if="modal.actions && modal.actions.length">
                      <button v-for="action in modal.actions" :key="action.label" 
                        @click="handleAction(action, modal)" 
                        :class="getActionButtonClass(action.type)">
                        {{ action.label }}
                      </button>
                    </template>
                    <button v-else @click="modalStore.removeModal(modal.id)" 
                  ...
                  <!-- Botón de editar del lado izquierdo -->
                  <div v-if="modal.type === 'bookingDetails' && modal.data?.bookingId" class="w-full sm:w-auto mb-2 sm:mb-0 sm:mr-auto">
                    <button @click="navigateToBookingEdit(modal.data.bookingId)"
