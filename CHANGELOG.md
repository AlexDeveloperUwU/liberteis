...
- Generated email icon PNGs (`emails/assets/`) were committed to the repo; they're build
  output from `npm run email:icons`, not hand-authored, so they're now gitignored instead.
  Regenerate them locally with `npm run email:icons` after installing; the Docker build now
  runs that step automatically. Moved `lucide-static` from `devDependencies` to
  `dependencies` so it's present when the production image installs with `--production`.
...
- `LoginView` showed no error feedback at all while typing an invalid email (e.g. `admin@`) — the
  previous fix made `:valid` live but left `:error` tied to state that was never set outside of a
  submit attempt, since blur validation can't reach `TextField`'s inner input. Replaced with
  `touched`/`submitted` flags driving both `emailError` and `passwordError` computeds live, and
  simplified `handleLogin` to check validity directly instead of re-running mutation-based
  validator functions.
